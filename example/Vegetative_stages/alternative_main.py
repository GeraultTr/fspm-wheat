...
def main(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
    ...
    if run_simu:
        ...
        try:
            ...
            outputs_df_dict = {}
            for outputs_df_list, outputs_filename, index_columns in ((wheat_fspm.axes_all_data_list, AXES_OUTPUTS_FILENAME, AXES_INDEX_COLUMNS),
                                                                     (wheat_fspm.organs_all_data_list, ORGANS_OUTPUTS_FILENAME, ORGANS_INDEX_COLUMNS),
                                                                     (wheat_fspm.hiddenzones_all_data_list, HIDDENZONES_OUTPUTS_FILENAME, HIDDENZONES_INDEX_COLUMNS),
                                                                     (wheat_fspm.elements_all_data_list, ELEMENTS_OUTPUTS_FILENAME, ELEMENTS_INDEX_COLUMNS),
                                                                     (wheat_fspm.soils_all_data_list, SOILS_OUTPUTS_FILENAME, SOILS_INDEX_COLUMNS)):
                outputs_filepath = os.path.join(OUTPUTS_DIRPATH, outputs_filename)
                outputs_df = pd.concat(outputs_df_list, keys=wheat_fspm.all_simulation_steps, sort=False)
    ...
    if run_postprocessing:
        ...
        if not run_simu:
            ...
            delta_t = wheat_fspm.CNWHEAT_TIMESTEP * HOUR_TO_SECOND_CONVERSION_FACTOR
